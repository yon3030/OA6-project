...
const Card: React.FC<CardProps> = ({ className, children, direction, link, cardImage, title, description, extraInfos, isSelected }) => {
    const verticalCard = () =>
        <Link href={link ? link : ""} className={`${className} flex flex-col bg-primary-default rounded-[20px] gradient-border gradient-border-rounded-20px ${isSelected ? "gradient-green-border" : "gradient-default-border"} relative justify-center items-center space-y-3 pb-2 hover:bg-[#151515] border-2 border-transparent hover:border-[#20d795]  active:shadow-[0_0_3px_3px_#20D795] transition-all duration-300`}>
    ...
    const horizontalCard = () =>
        <Link href={link ? link : ""} className={`${className} flex flex-row bg-primary-default rounded-[20px] gradient-border gradient-border-rounded-20px ${isSelected ? "gradient-green-border" : "gradient-default-border"}  relative p-3 mb-3 hover:bg-[#151515] transition-all duration-300`}>
