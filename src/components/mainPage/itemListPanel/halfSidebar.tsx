...
import { ItemListPanelProps } from "./type";

const ItemListPanel: React.FC<ItemListPanelProps> = ({ className, title, children, expandVisible }) => {
    const [isExpanded, setExpanded] = useState(false);

    return (
        <div className={`flex flex-row`}>
            <div
                className={`${className} ${isExpanded ? "w-[630px]" : "w-[330px]"}`}
            >
                ...
                />
                <div className={`max-h-[600px] px-8 transition-all duration-300 justify-between w-full overflow-y-auto overflow-x-hidden my-[14.25px] scrollbar-thin scrollbar-green`}>
                    <div className={`grid gap-2 ${isExpanded ? 'grid-cols-2' : 'grid-cols-1'} transition-all duration-300`}>
                        {children}
                    </div>
                </div>
            </div>
